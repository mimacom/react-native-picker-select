...
| `items`                                         | The items for the component to render<br> - Each item should be in the following format:<br>`{label: 'Orange', value: 'orange', key: 'orange', color: 'orange', contentDescription: 'Orange Item', inputLabel: 'Orange!', testID: 'orange'}`<br>- `label` and `value` are required<br>- `key`, `color`, `contentDescription`, `testID` and `inputLabel` are optional<br>- `key` will be set to equal `label` if not included<br>- `value` can be any data type<br>- If `inputLabel` exists, the TextInput will display that value instead of the `label`                                    | **required**<br>array    |
